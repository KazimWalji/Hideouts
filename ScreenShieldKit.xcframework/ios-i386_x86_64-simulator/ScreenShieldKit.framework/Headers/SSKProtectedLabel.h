...
/**
 A screenshot-protected replacement for UILabel. Note that `SSKProtectedLabel` is a subclass of UIView, not UILabel, so be considerate of this when replacing your existing labels in Interface Builder.

 `SSKProtectedLabel` can NOT be used to “cover over” content in your app to protect everything underneath from screenshots or screen captures. Only the text or attributed text provided to the initializer or the `text` or `attributedText` properties will be protected from screenshots and screen captures.
