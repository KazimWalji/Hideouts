...
class ChatNetworking {
    ...
    func uploadGifImage(gifData: Data, completion: @escaping (_ storageRef: StorageReference, _ gifData: Data, _ name: String) -> Void) {
            
            let mediaName = NSUUID().uuidString
            let storageRef = Storage.storage().reference().child("message-gifs").child(mediaName)
           
            let uploadTask = storageRef.putData(gifData, metadata: nil) { (metadata, error) in
                if let error = error {
                    print(error.localizedDescription)
                    return
                }
                
                return completion(storageRef, gifData, mediaName)
    ...
    func uploadImage(image: UIImage, completion: @escaping (_ storageRef: StorageReference, _ image: UIImage, _ name: String) -> Void){
       // SSKProtectedImageView(image: image)
        let mediaName = NSUUID().uuidString
    ...
    func downloadGifImage(_ ref: StorageReference, _ image: UIImage, _ id: String) {
        ref.downloadURL { (url, error) in
            guard let url = url else { return }
