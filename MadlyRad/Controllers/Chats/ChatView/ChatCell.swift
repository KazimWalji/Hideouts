...
class ChatCell: UICollectionViewCell {
  ...
  //  let message = SSKProtectedLabel(text: "")
    ...
    /*
     ...
     let message = SSKProtectedLabel(text: msg!.message)
     backgroundWidthAnchor.constant = chatVC.calculateFrameInText(message: msg!.message ?? "").width + 32
     


     */
  //  let asd = SSKProtectedLabel(text: msg?.message ?? "")
    var msg: Messages? {
    ...
    override func prepareForReuse() {
        ...
        if messageBackground.subviews.count > 4{
             self.messageBackground.subviews[4].removeFromSuperview()
            }
    ...
    func setupMessagePosition(_ msg: Messages?) {
    ...
    private func setupCustomGifMessage(_ msg: Messages?) {
        
        if msg?.mediaUrl != nil {
            if msg?.mediaUrl.contains("gifs") == true {
                
                //var testImage = UIImage.gifImageWithURL(msg!.mediaUrl)
               
                let url = URL(string: msg!.mediaUrl)!
                mediaMessage.setGifFromURL(url)
                /*var testImage = UIImage.gifImageWithURL(msg!.mediaUrl)
                self.mediaMessage.animationImages = (testImage?.images)!
                self.mediaMessage.animationDuration = (testImage?.duration)!
                self.mediaMessage.image = testImage?.images.last
                self.mediaMessage.startAnimating()*/
    ...
    private func setupGifMessage(_ msg: Messages?){
        if msg?.media != nil{
            backgroundWidthAnchor.constant = 200
           // backgroundHeightAnchor.constant = 300
            let mediaView = GPHMediaView()
            let id = msg?.media
            GiphyCore.shared.gifByID(id!) { (response, error) in
                if let media = response?.data{
                    DispatchQueue.main.sync { [weak self] in
                        mediaView.media = media             //convert id to gif object
                        //self!.messageBackground.layer.addSublayer(CALayer(layer: mediaView))
                        mediaView.frame = self!.messageBackground.bounds
                        self!.messageBackground.addSubview(mediaView) //display gif
