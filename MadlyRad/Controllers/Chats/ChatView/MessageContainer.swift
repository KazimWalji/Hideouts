...
class MessageContainer: UIView, UITextViewDelegate{
    ...
    let cancelMicButton = UIButton(type: .system)
    ...
    private func cancelAudioAction(){
        addSubview(cancelMicButton)
        cancelMicButton.isHidden = true
        cancelMicButton.translatesAutoresizingMaskIntoConstraints = false
        cancelMicButton.setImage(UIImage(systemName: "stop"), for: .normal)
        cancelMicButton.tintColor = .black
        cancelMicButton.addTarget(chatVC, action: #selector(chatVC.cancelAudioRecordingAction), for: .touchUpInside)
        let constraints = [
            cancelMicButton.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -const),
            cancelMicButton.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -30),
            cancelMicButton.heightAnchor.constraint(equalToConstant: 30),
            cancelMicButton.widthAnchor.constraint(equalToConstant: 30),        ]
        NSLayoutConstraint.activate(constraints)
    }
    ...
    private func recordingAudioAnimation(){
        ...
        recordingAudioView.animation = Animation.named("12634-audio-play")
        ...
        let constraints = [
            ...
            recordingAudioView.heightAnchor.constraint(equalToConstant: 50),
            recordingAudioView.widthAnchor.constraint(equalToConstant: 50)
        ]
        NSLayoutConstraint.activate(constraints)
    }
