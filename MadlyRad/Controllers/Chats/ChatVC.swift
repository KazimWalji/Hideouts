...
class ChatVC: UIViewController,UIImagePickerControllerDelegate, UINavigationControllerDelegate, AVAudioRecorderDelegate, GiphyDelegate, SKPaymentTransactionObserver, SKProductsRequestDelegate{
    ...
    private var hideChatController: HideChatController?
    ...
    */
    ...
    @objc func clipImageButtonPressed() {
        ...
        let paidstickers = UIAlertAction(title: NSLocalizedString("paid stickers!", comment: ""), style: .default, handler: { _ in
            let storyboard = UIStoryboard(name: "HomeVC", bundle: nil)
                               let controller = storyboard.instantiateViewController(withIdentifier: "PaidStickerVC") as! PaidStickerVC
                               self.present(controller, animated: true, completion: nil)
                               controller.sendSticker = {image in
...
*/
        ...
        self.present(controller, animated: true, completion: nil)
            ...
            controller.sendGif = {gifData in
                self.chatNetworking.uploadGifImage(gifData: gifData) { (storageRef, gifData, mediaName) in
                    self.chatNetworking.downloadGifImage(storageRef, UIImage.gifImageWithData(gifData)!, mediaName)
                }
                return 0
            }
    ...
    @objc func cancelAudioRecordingAction(){
    ...
    private func prepareContainerForRecording(){
        ...
        UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0, options: .curveEaseIn, animations: {
            ...
            self.messageContainer.cancelMicButton.isHidden = false
        }) { (true) in
    ...
    private func removeRecordingUI(){
        ...
        messageContainer.cancelMicButton.isHidden = true
        ...
        UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0, options: .curveEaseOut, animations: {
            ...
            self.messageContainer.cancelMicButton.isHidden = true
...
extension ChatVC: HideChatType {
    ...
    func setupHideKeyboardTap(isActive:Bool) {
        if isActive {
            hideKeyboardTap = UITapGestureRecognizer(target: self, action: #selector(hideKeyboard))
            self.view.addGestureRecognizer(hideKeyboardTap!)
        } else {
            if hideKeyboardTap != nil {
