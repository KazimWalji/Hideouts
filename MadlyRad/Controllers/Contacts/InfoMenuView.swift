  ...
  //  let infoEmail = UILabel()
    ...
    func startingAnimationInfoMenu(){
        ...
        UIView.animate(withDuration: 0.2, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: .curveEaseOut, animations: {
            ...
            self.infoUserID.layer.add(self.moveObjectToCenter(xValue: 60, yValue: 130, yConst: 160), forKey: "MoveEmailToTheCenter")
        }) { (true) in
    ...
    private func setupInfoName() {
        ...
        infoName.font = UIFont(name: "Proxima Nova", size: 24)
        let constraints = [
    ...
    private func setuoInfoUserID() {
        addSubview(infoUserID)
        infoUserID.translatesAutoresizingMaskIntoConstraints = false
        infoUserID.text = friend.UserID
        infoUserID.textAlignment = .center
        infoUserID.textColor = .gray
        infoUserID.font = UIFont(name: "Proxima Nova", size: 16)
        let constraints = [
            infoUserID.topAnchor.constraint(equalTo: infoName.bottomAnchor),
            infoUserID.leadingAnchor.constraint(equalTo: infoImage.trailingAnchor, constant: 15)
        ]
        NSLayoutConstraint.activate(constraints)
    }
