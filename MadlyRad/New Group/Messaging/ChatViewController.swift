...
final class ChatViewController: MessagesViewController {
    ...
    private var selfSender: Sender? {
        ...
        return Sender(photoURL: "",
                      senderId: safeEmail,
                      displayName: "Me", sentDate: Date())//dateString
    }
   ...
   // self.messagesCollectionView.contentInset = UIEdgeInsets(top: 50, left: 0, bottom: 50, right: 0)
  ...
  //  }
    ...
    override func viewDidLoad() {
        ...
        setupInputButton()
        didTapMessage(in: MessageCollectionViewCell())
        let backgroundImage = UIImage(named: "backgroundimageforhideouts.png")
        let imageView = UIImageView(image: backgroundImage)
        self.messagesCollectionView.backgroundView = imageView
          NotificationCenter.default.addObserver(
                     forName: UIApplication.userDidTakeScreenshotNotification,
                     object: nil, queue: nil) { _ in
                       print("I see what you did there4")
                   }    }
    private func messagesCollectionView() {
    ...
    private func presentInputActionSheet() {
        ...
        actionSheet.addAction(UIAlertAction(title: "Sticker", style: .default, handler: { [weak self] _ in
        self?.presentStickerInputActionsheet()
        }))
...
//STICKERS!!!!
    private func presentStickerInputActionsheet() {
        let actionSheet = UIAlertController(title: "Attach Video",
                                            message: "Where would you like to attach a video from?",
                                            preferredStyle: .actionSheet)
        actionSheet.addAction(UIAlertAction(title: "Camera", style: .default, handler: { [weak self] _ in
    ...
    messageInputBar.inputTextView.becomeFirstResponder()
        if let conversationId = conversationId {
            listenForMessages(id: conversationId, shouldScrollToBottom: true)//true
        }
    }
...
extension ChatViewController: MessageCellDelegate {
    ...
    func inputBar6(_ inputBar: InputBarAccessoryView, didTapMessage text: String) {
    guard !text.replacingOccurrences(of: " ", with: "").isEmpty,
        let selfSender = self.selfSender,
        let messageId = createMessageId() else {
            text.replacingOccurrences(of: " ", with: "").isEmpty
