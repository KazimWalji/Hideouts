...
class UsersListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate{
    ...
    override func viewDidLoad() {
        ...
        //ref!.child("users").child(userID!).child("pin").removeValue()
...
func filterSearch(searchText: String) {
        ...
        DispatchQueue.global(qos: .userInteractive).async { [weak self] in
            ...
            self?.users = self?.users.filter({
                let match = $0.pin?.elementsEqual(searchText)
                print(match)
                
                //let match = $0.pin.equals(other: searchText)
                //CHANGE BOTTOM LINE TO return match = true
                return match == true
            }) ?? [FriendInfo]()
